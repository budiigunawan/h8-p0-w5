...
RULES:
Dilarang menggunakan built-in function kecuali .push(), boleh pake toString()
  ...
  for (var i = 0; i < pirates.length; i++) {
    ...
    if (cek == true) {
      for (var key in lokasi) {
        if (pirates[i].location == key) {
          var tahun = String(2020 - pirates[i].age)
          var id = tahun[2] + tahun[3] + '-' + lokasi[key]
          var obj = {
            name : pirates[i].name,
            ID : id
          }
